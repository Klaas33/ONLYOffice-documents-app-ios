...
# release/v9.3.0 is only the package branch name. Its immutable manifest uses
# the public hotfix/v9.2.1/181 binaries; it is not a v9.3 editor engine.
PACKAGE_REVISION=0c584d936384402f090f1d66823c42186a2ed972
...
PACKAGE_DIR=${ONLYOFFICE_PACKAGE_DIR:-"$ROOT/.local-artifacts/editors-ios-sp-hotfix-v9.2.1-181"}
TEMPLATE_DIR=${ONLYOFFICE_TEMPLATE_DIR:-"$(dirname "$ROOT")/document-templates"}
ARTIFACT_DIR=${ONLYOFFICE_ARTIFACT_DIR:-"$HOME/Library/Caches/ONLYOFFICE/editors-hotfix-v9.2.1-181"}
DERIVED_DATA=${DERIVED_DATA:-"$ROOT/.local-artifacts/DerivedData"}
BUNDLER_VERSION=${BUNDLER_VERSION:-2.7.1}
...
# Install the Bundler version recorded by this v9.2 app snapshot so CocoaPods
# resolution uses the checked-in lockfile without mutating dependency versions.
